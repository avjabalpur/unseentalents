...
import type { Event, PublicSummary } from "@/types/api";
...
export default async function HomePage() {
  const [events, summary] = await Promise.all([getEvents(), getSummary()]);

  return (
    <div>
      {/* Hero carousel (HeroSlider, admin-managed slides) disabled for now in favor of a static hero */}
      <section
      ...
      >
        <div className="mx-auto max-w-[1600px] px-4 py-28 text-center sm:py-36">
          <p className="mb-4 flex items-center justify-center gap-2 text-sm font-semibold uppercase tracking-[0.3em] text-primary">
            <span className="size-1.5 rounded-full bg-primary shadow-[0_0_10px_1px_rgba(var(--brand-rgb),0.8)]" />
            SecretWhiz
          </p>
          <h1 className="font-heading text-5xl font-bold tracking-tight text-balance text-white sm:text-7xl">
            Show your talent. <span className="text-primary">Get discovered.</span>
          </h1>
          <p className="mx-auto mt-5 max-w-xl text-lg text-white/70 text-balance">
